...
export function Sidebar({ apiKey, onSignOut }: Props) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('nqita:sessions', handleUpdate);
    return () => window.removeEventListener('nqita:sessions', handleUpdate);
  }, []);

  const onNewChat = () => {
    window.dispatchEvent(new CustomEvent('nqita:new-chat'));
  };

  const onSelectSession = (id: string) => {
    window.dispatchEvent(new CustomEvent('nqita:select-session', { detail: id }));
  };

  const onDeleteSession = (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    window.dispatchEvent(new CustomEvent('nqita:delete-session', { detail: id }));
  };
